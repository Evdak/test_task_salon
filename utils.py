...
def check_timedelta_before(time: datetime = datetime.now()) -> datetime:
    return time - timedelta(minutes=DEFAULT_TIME)


def check_timedelta_after(time: datetime = datetime.now()) -> datetime:
    return time + timedelta(minutes=DEFAULT_TIME)
...
def get_open_datetime() -> datetime:
    ...
    return datetime.combine(date=datetime.now().date(), time=time(
        hour=open_time[0], minute=open_time[1]), tzinfo=utc)
...
def get_close_datetime() -> datetime:
    ...
    return datetime.combine(date=datetime.now().date(), time=time(
        hour=close_time[0], minute=close_time[1]), tzinfo=utc)


def get_live_queue_max_count() -> int:
    result = get_close_datetime() - datetime.now(tz=utc)
